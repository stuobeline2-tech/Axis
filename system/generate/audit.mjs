...
export function renderAudit({ practice, summary, decision, ingestReport, asOf, preparedBy }) {
  ...
  if (summary.counts.window_unknown > 0) {
    p(`> ${summary.counts.window_unknown} claims sit with payers whose appeal window we have not documented.`);
    ...
    p('> these move into one of the two rows above.');
    p('');
  }
  ...
  if (!summary.worklist.length) {
    ...
    p(`Top ${summary.worklist.length} recoverable claims by value.`);
    p('');
  ...
  if (summary.counts.unknown_code > 0) p(`- The meaning of ${summary.counts.unknown_code} claims whose denial code is outside our reference set.`);
  if (summary.counts.window_unknown > 0) p(`- The filing deadline for ${summary.counts.window_unknown} claims (see §2).`);
  if (ingestReport?.rejected?.length) {
